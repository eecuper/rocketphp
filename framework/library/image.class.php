...
class image {
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'hx_cms',$w_font = 8, $w_color = '#ff0000') {
		$w_pos = $w_pos ? $w_pos : $this->w_pos;
		$w_img = $w_img ? $w_img : $this->w_img;
		if(!$this->watermark_enable || !$this->check($source)) return false;
		if(!$target) $target = $source;
		$w_img = APPLICATION.'/static/images/water/'.$w_img;
		...
		if(!empty($w_img) && is_file($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, SYS_ROOT.'fonts/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
	...
	function check($image) {
		...
		&& is_file($image) && function_exists('imagecreatefrom'.($m[1] == 'jpg' ? 'jpeg' : $m[1]));
	}
}
